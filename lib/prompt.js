...
module.exports = function () {
  ...
  var getUserCompletions = function getUserCompletions(line, word, options, cb) {
    ...
    result = !queryWord ? null : request.get(config.auth.url + 'rest/api/2/user/search?username=' + queryWord).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
      if (!res.ok) {
