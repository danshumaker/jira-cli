...
module.exports = (() => {
  ...
  function _extractErrorMessages (response) {
    const { errors, messages, errorMessages } = typeof response === 'string' ? JSON.parse(response).body : response.body;
    ...
    return (messages && messages.length) ? messages : (errorMessages && errorMessages.length) ? errorMessages : convertErrorsToArray(errors);
  }
