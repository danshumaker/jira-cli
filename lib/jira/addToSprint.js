...
module.exports = function () {
  ...
  function addToSprint(sprintId, projIsssue, cb) {
    ...
    request.put(config.auth.url + '/rest/api/2/issue/' + projIsssue).send(data).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
      if (!res.ok) {
