...
module.exports = function () {
  ...
  var worklog = {
    add: function (issue, timeSpent, comment, startedAt) {
      ...
      request.post(config.auth.url + url).send({
        comment: comment,
        timeSpent: timeSpent,
    ...
    show: function (issue) {
      var url = 'rest/api/latest/issue/' + issue + '/worklog';
      request.get(config.auth.url + url).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
        if (!res.ok) {
