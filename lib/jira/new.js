...
module.exports = function () {
  ...
  var create = {
    ...
    getMeta: function (key, callback) {
      ...
      request.get(config.auth.url + this.query).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
        if (!res.ok) {
    ...
    saveIssue: function (options, cb) {
      ...
      post_url = config.auth.url + this.query; //create.answers.fields.reporter['name'] = create.answers.fields.reporter['id'];
      ...
      request.post(post_url).send(body).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
        if (!res.ok) {
          if (options.verbose) {
            console.log(res);
          }
        ...
        console.log('Open ' + url.resolve(config.auth.url, 'browse/' + res.body.key));
        return cb();
      });
    },
