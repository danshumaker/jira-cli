...
module.exports = function () {
  ...
  var getRapidBoardID = function getRapidBoardID(rapidBoardID, cb) {
    ...
    request.get(config.auth.url + 'rest/greenhopper/latest/rapidviews/list').set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
      if (!res.ok) {
  ...
  var getSprintID = function getSprintID(rapidBoardID, sprintID, cb) {
    ...
    request.get(config.auth.url + 'rest/greenhopper/latest/sprintquery/' + rapidBoardID).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
      if (!res.ok) {
  ...
  var getSprintIssues = function getSprintIssues(rapidBoardID, sprintID, cb) {
    ...
    request.get(config.auth.url + 'rest/greenhopper/latest/rapid/charts/sprintreport?' + qParams).set('Content-Type', 'application/json').set('Authorization', 'Basic ' + config.auth.token).end((err, res) => {
      if (!res.ok) {
